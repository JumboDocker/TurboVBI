...
for snr_idx = 1:length(SNR_dB_range)
    ...
    for trial = 1:trials
        ...
        %% === Support-VBI with adaptive π ===
        mu_x = zeros(N,1); Sigma_x = eye(N);
        a_hat = zeros(N,1); b_hat = zeros(N,1);
        E_alpha = ones(N,1);
        s_hat = 0.05 * ones(N,1); % initial π based on sparsity
        alpha_inactive = 1e2;

        pi_evolution = zeros(N, max_iter); % for 3D plot
        ...
        nmse_support(trial, snr_idx) = norm(mu_x - x_true)^2 / (norm(x_true)^2 + 1e-6);
        ...
        nmse_baseline(trial, snr_idx) = norm(mu_x - x_true)^2 / (norm(x_true)^2 + 1e-6);
        ...
        for iter = 1:max_iter
            ...
            E_x2 = mu_x.^2 + diag(Sigma_x);
            for i = 1:N
                a_hat(i) = s_oracle(i) * a0_act + (1 - s_oracle(i)) * a0_inact + 1;
        ...
        nmse_oracle(trial, snr_idx) = norm(mu_x - x_true)^2 / (norm(x_true)^2 + 1e-6);
...
figure;
[X,Y] = meshgrid(1:max_iter, 1:N);
surf(X,Y,save_visual.pi_evolution, 'EdgeColor', 'none'); view(-30,30);
xlabel('Iteration'); ylabel('Index'); zlabel('\pi');
title('\pi Evolution in Support-VBI');

figure;
subplot(3,1,1); stem(save_visual.s_true, 'k'); title('True Support');
subplot(3,1,2); stem(save_visual.s_hat > 0.5, 'r'); title('Estimated Support (Support-VBI)');
